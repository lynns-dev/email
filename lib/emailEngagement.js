...
export function resolveSegment(subscribers, segment, now = Date.now()) {
  ...
  // Leads (source: 'lead'), checkout starters, and newsletter signups who
  // haven't converted yet — !lastOrderAt covers all of them regardless of
  // how they entered the list, rather than matching on source alone.
  if (segment === 'no_purchase') return subscribed.filter((s) => !s.lastOrderAt);
