...
// Veil's own brand ink/soft (lib/theme.js in the veil-ecommerce repo) —
// this is the default color for any content that doesn't set its own
// (the footer, unstyled fallback text), not just a generic gray.
const INK = '#16140F';
const SOFT = '#8B8678';
  ...
  const rows = items
    .map((item) => {
      const img = item.image
        ? `<img src="${escapeHtml(item.image)}" alt="" style="width:64px;height:64px;object-fit:cover;border-radius:4px;display:block;" />`
        : '';
