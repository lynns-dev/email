...
// by pages/api/email/checkout-capture.js for the storefront's own
// checkout-page consent checkbox. Suppressed addresses are left alone
// rather than reactivated — same rule as upsertFromShopify: a
// bounce/complaint is a deliverability signal independent of anyone
// re-asserting consent.
export async function addSubscriberManually(email, source = 'manual') {
  const normalized = normalizeEmail(email);
