...
// Added directly by the admin (source: 'manual') — skips double opt-in,
// since adding someone here is the admin asserting they already have a
// lawful basis to email them (an existing customer, a re-imported list,
// etc.), the same trust boundary upsertFromShopify already relies on for
// its already-consented Shopify customers. Suppressed addresses are left
// alone rather than reactivated — same rule as upsertFromShopify: a
// bounce/complaint is a deliverability signal independent of anyone
// re-asserting consent.
export async function addSubscriberManually(email) {
  const normalized = normalizeEmail(email);
