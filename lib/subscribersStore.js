...
// customers webhook does, so this never creates a new subscriber). Also
// clears any in-progress checkout — an order means it converted, so the
// abandoned-checkout automation shouldn't treat it as abandoned.
...
// this app — it only acts on people already on the list.
export async function updateCartActivity(email, cartValue, itemCount) {
  ...
  subscribers[idx] = { ...subscribers[idx], cartValue, cartItemCount: itemCount, cartUpdatedAt: Date.now() };
  await saveSubscribers(subscribers);
  return subscribers[idx];
}
...
// gets its own fresh run instead of picking up a stale step index.
export async function recordCheckoutStarted(email, cartValue) {
  ...
  subscribers[idx] = {
    ...
    automationState: { ...subscribers[idx].automationState, abandoned_checkout: { step: 0 } },
  };
  await saveSubscribers(subscribers);
  return subscribers[idx];
}
