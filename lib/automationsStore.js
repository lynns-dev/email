...
const SEED_AUTOMATIONS = [
  ...
  },
  {
    ...
    enabled: true,
    // Hours (fractional for the first step), not days — cart abandonment
    // is time-sensitive in a way the other flows aren't. See DEPLOYMENT.md
    // for why this flow specifically needs a cron/pinger running more
    // often than hourly to actually land close to the 30-minute target.
    steps: [
      {
        delayHours: 0.5,
        ...
        html: eyebrow('Your cart', 'center')
          ...
          + image('Veil', 'https://veilpuff.com/images/veil-puff-shoulder-hoop.webp')
          + p("Your cart's exactly how you left it — pick up right where you stopped.", 'center')
          + '{{CART_ITEMS}}'
      ...
      },
      {
        ...
        html: eyebrow('10% off, on us', 'center')
          + heading('One more reason to say yes.', 'center')
          + image('Veil', 'https://veilpuff.com/images/veil-grand-jar.png')
          + p("Still on the fence? Here's 10% off to help you decide.", 'center')
          + '{{CART_ITEMS}}'
          + code('WELCOME10')
