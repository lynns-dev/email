// Per-campaign conversion rate: of the subscribers who clicked a link in
// a campaign, what fraction placed an order within 7 days of that click.
// Deliberately click-based (not open-based) as the "did the campaign
// work" signal, same reasoning as lib/emailEngagement.js's engagement
// tiering — Apple Mail Privacy Protection auto-fires opens for nearly
// everyone regardless of whether they looked, so open rate isn't a
...
// this campaign's 7-day window — an inherent gap in the current data
// model, not a bug in this calculation.
...
export async function computeCampaignConversion(campaignId, subscribersByEmail) {
  ...
  // A subscriber can click a campaign's links more than once — only the
  // first click starts their conversion window.
